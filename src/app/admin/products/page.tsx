  ...
  return (
    <>
      <header className="flex items-center justify-between">
        <h1 className="text-navy text-2xl font-bold dark:text-white">제품 관리</h1>
        <Link
      ...
      {items.length === 0 ? (
        <p className="text-gray-dark mt-8 dark:text-gray-400">등록된 제품이 없습니다.</p>
      ) : (
        <>
          {/* 모바일: 카드 */}
          <ul className="mt-6 space-y-3 md:hidden">
            {items.map((item) => (
              ...
              >
                <p className="text-navy font-semibold dark:text-white">{item.name}</p>
                <p className="text-gray-dark mt-1 text-sm dark:text-gray-300">
                  {item.category} · 특징 {item.features.length}개
                </p>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <td className="text-navy py-2 pr-4 dark:text-white">{item.name}</td>
