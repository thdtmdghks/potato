...
export default async function AdminInquiries({
  ...
  return (
    <>
      <h1 className="text-navy text-2xl font-bold dark:text-white">문의 관리</h1>
      ...
      <nav className="mt-4 flex gap-2 overflow-x-auto pb-2" aria-label="상태 필터">
        {[
          ...
          {
            href: "/admin/inquiries?status=confirmed",
            label: "확인",
          ...
          <Link
            key={f.label}
            href={f.href}
            className={`shrink-0 rounded px-3 py-2 text-sm ${f.active ? "bg-navy text-white" : "bg-gray-light text-gray-dark dark:bg-gray-800 dark:text-gray-300"}`}
          >
            {f.label}
          </Link>
        ))}
      ...
      {items.length === 0 ? (
        <p className="text-gray-dark mt-8 dark:text-gray-400">문의가 없습니다.</p>
      ) : (
          ...
          {items.map((inq) => (
            <li key={inq.id}>
              <Link
                ...
                className="block rounded-lg border border-gray-200 p-4 transition-shadow hover:shadow dark:border-gray-700"
              >
                ...
                <p className="text-gray-dark mt-1 line-clamp-1 text-sm dark:text-gray-400">
                  {inq.content}
                </p>
                <p className="text-gray-dark mt-1 text-xs dark:text-gray-500">
