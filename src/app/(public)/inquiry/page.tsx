...
export default function Inquiry() {
  ...
  if (submitted) {
    return (
      <section className="py-16 text-center">
        <h1 className="text-navy text-3xl font-bold dark:text-white">문의가 접수되었습니다</h1>
        <p className="text-gray-dark mt-4 dark:text-gray-300">빠른 시일 내에 연락드리겠습니다.</p>
      </section>
    );
  }
  ...
  return (
    <>
      <h1 className="text-navy text-3xl font-bold dark:text-white">견적문의</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="mt-8 max-w-2xl space-y-6">
        ...
        <div>
          <label
            htmlFor="files"
            className="text-navy mb-1 block text-sm font-medium dark:text-gray-200"
          >
            참고 이미지{" "}
            <span className="text-gray-dark text-xs dark:text-gray-400">
              (최대 {MAX_FILES}장, 자동 압축)
            </span>
          </label>
          <input
            ref={fileRef}
          ...
          />
          {compressing && (
            <p className="text-gray-dark mt-1 text-xs dark:text-gray-400">압축 중...</p>
          )}
          {files.length > 0 && (
            <ul className="mt-2 space-y-1">
              {files.map((f, i) => (
                <li
                  key={i}
                  className="bg-gray-light flex items-center justify-between rounded px-3 py-1 text-sm dark:bg-gray-800"
                >
                  <span className="text-gray-dark truncate dark:text-gray-300">
        ...
        <button
          ...
          className="bg-navy hover:bg-navy-light w-full rounded-lg px-8 py-3 font-semibold text-white transition-colors disabled:opacity-50 sm:w-auto"
        >
...
function Field({
  ...
  return (
    <div>
      <label htmlFor={id} className="text-navy mb-1 block text-sm font-medium dark:text-gray-200">
        {label}
        {required && (
      ...
      {children}
      {error && (
        <span id={errorId} role="alert" className="mt-1 block text-xs text-red-500">
          {error}
        </span>
      )}
    </div>
