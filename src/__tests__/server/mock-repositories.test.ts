...
describe('MockProjectRepository', () => {
  ...
  beforeEach(() => { repo = new MockProjectRepository(); });
  ...
  it('create는 새 항목을 추가한다', async () => {
    ...
    const created = await repo.create({ title: '새 프로젝트', description: '설명', category: '웹', images: [] });
...
describe('MockProductRepository', () => {
  ...
  it('CRUD가 동작한다', async () => {
    const created = await repo.create({ name: '신규', description: '설명', category: '앱', image: '', features: [] });
    expect(created).not.toBeNull();
...
describe('MockInquiryRepository', () => {
  ...
  beforeEach(() => { repo = new MockInquiryRepository(); });

  it('create는 status를 pending으로 설정한다', async () => {
    const created = await repo.create({ name: '테스트', phone: '010-0000-0000', email: null, type: '웹', address: '서울', content: '문의' });
