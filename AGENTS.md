...
- 컴포넌트 및 로직 모듈화:
  ...
  - 리렌더링 최적화: 폼 필드처럼 잦은 입력 변경이 발생하는 상태(city, town 등)는 서브 컴포넌트 내부로 격리하여 부모 컴포넌트 전체의 불필요한 리렌더링을 차단합니다. 폼 제출 시점 혹은 단발성으로만 필요로 하는 값(AI 생성용 특이사항 등)은 `useState` 대신 `useRef`를 사용하여 실시간 리렌더링 횟수를 0회로 격리 최적화합니다.
