...
불필요하게 부모 컴포넌트가 반복적으로 리렌더링되는 것을 방지하기 위해 다음 원칙을 준수한다.

- **상태 고립 (상태 범위 최소화):** 자주 업데이트되는 입력 상태(예: 상세 주소 입력 시 city, town 로컬 상태 등)는 독립된 서브 컴포넌트(`RegionSelector` 등) 내부로 격리하여 타이핑 시 부모 컴포넌트 전체가 다시 렌더링되지 않도록 한다.
- **제출 시점 전용 값은 useRef 사용:** 폼 제출 시점이나 특정 버튼 클릭 시점에만 동적으로 읽으면 되는 입력값(예: AI 설명 자동 추천용 특이사항 등)은 `useState` 대신 `useRef`를 사용하여 글자를 입력할 때의 불필요한 리렌더링 횟수를 **0회**로 최적화한다.
- **복잡한 렌더러 격리:** 이미지 목록 렌더링, 진행 바 등 리렌더링 주기가 다르거나 비용이 큰 UI 요소는 상위 상태와 격리된 서브 컴포넌트로 캡슐화한다.
...
- [ ] 불필요한 리렌더링을 차단하기 위한 상태 격리 및 useRef 최적화를 고려했는가?
