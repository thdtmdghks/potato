...
와이어프레임 프로토타입 완료 (9/9). develop 브랜치 `449df5c`.
...
3. ✅ **포트폴리오 목록/상세** — picsum 이미지 표시, 이미지 없음 fallback, 반응형 그리드(1→2→3열), 카테고리 필터(가로 스크롤), 상세 갤러리(최대 8장 + 나머지 카운트).
4. ✅ **제품 안내** — 이미지 있음/없음 분기, features 전체 표시, 반응형 2열 그리드.
5. ✅ **견적문의 폼** — react-hook-form + zod(inquirySchema), `"use client"`. id/htmlFor/aria-describedby 명시적 연결. fieldset 안 div 래퍼.
6. ✅ **회사소개/연락처** — 비전, 핵심 가치(3개 카드), 연혁 타임라인. 연락처 정보(dl/dt/dd), 지도 placeholder.
7. ✅ **관리자 대시보드** — 요약 카드 3개, 모바일 카드형 + 데스크톱 테이블. th scope="col".
8. ✅ **다크모드 토글** — ThemeToggle(☀️/🌙/💻 순환), mounted 전 아이콘 숨김.
9. ✅ **빌드/테스트 확인 및 커밋** — 14파일 +653/-107. 빌드 ✅, 테스트 26개 ✅.
...
- 테이블 `<th scope="col">` 추가
- 미사용 `stats.href` 제거
- 터치 타겟 44px (카테고리 필터, 햄버거 버튼, 모바일 메뉴 링크)
- 이미지 `h-48` → `aspect-4/3` 비율 기반
- 관리자 테이블 → 모바일 카드형 + 데스크톱 테이블
- 동적 aria-label (햄버거 메뉴 열기/닫기)
- AGENTS.md에 Mobile First 섹션 + 접근성 규칙 추가

### 다음 작업 (도메인 확정 후)
11. 콘텐츠 교체 (더미→실제 데이터)
12. 페이지별 메타데이터 설정
13. JSON-LD 구조화 데이터
14. sitemap.xml / robots.txt / canonical
15. SEO 검증 및 커밋
16. 성능 최적화 및 접근성 검사
