...
- [x] **Phase 3**: 개인정보 처리방침 페이지 및 푸터 연동 (Commit 3)
- [x] **Phase 4**: 관리자 페이지: 리뷰 초대 링크 생성 및 카톡 공유 (Commit 4)
- [x] **Phase 5**: 후기 작성/수정 UI 및 비즈니스 로직 (Commit 5)
- [x] **Phase 6**: 마이페이지: 내가 쓴 후기 확인 (Commit 6)
...
### [ ] 후기 상세 보기 모달(Modal) 팝업 기능 도입

- **배경**: 메인 화면 캐러셀 카드는 격자 규격 일관성을 유지하기 위해 `line-clamp-4`로 텍스트를 제한하여 4줄까지만 노출합니다. 이보다 긴 후기는 뒷부분이 잘려서 노출됩니다.
- **요구사항**:
  - 카드 클릭 또는 4줄 초과 시 활성화되는 `[더보기]` 인터랙션을 통해 **상세 보기 팝업 모달**을 띄웁니다.
  - 모달 내에서 긴 리뷰 글 전문을 스크롤 제약 없이 정독할 수 있도록 구현합니다.
  - 첨부된 여러 장의 시공 완료 사진들을 큼직한 원본 해상도 슬라이더(캐러셀) 형태로 감상할 수 있는 고화질 상세 뷰어 오버레이를 제공합니다.
