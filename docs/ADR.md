...
## [미래] 리뷰 시스템 v2 — 단일 테이블 버전 관리

**상태:** 미착수 (현재 구조에서 문제 발생 시 진행)

**현재 구조의 한계:**

- `reviews` + `review_edits` 2테이블 구조
- 수정 승인 시 원본을 덮어쓰고 `review_edits`를 삭제 → 이전 원본 이력 소실
- 하드삭제 없어도 업데이트 히스토리 추적 불가

**제안 구조:**

- 테이블 1개 (`reviews`)에 `version` 컬럼 추가
- 신규 작성 → version 1, pending
- 승인 → version 1, approved
- 수정 요청 → version 2, pending (기존 version 1은 approved 유지)
- 수정 승인 → version 2, approved (version 1은 이력으로 보존)
- 현재 노출본 = 가장 높은 version 중 approved인 행

**장점:** 이력 전부 보존, 테이블 1개, RLS 단일 관리
**단점:** "최신 approved" 쿼리 복잡도 증가, 기존 코드 전면 재설계 필요

---

## ADR-016: 5단계 AI 시공사례 포트폴리오 설명 재생성 및 브라우저 뒤로가기 가드 연동

**배경:**
관리자가 시공사례를 새로 등록하거나 수정할 때, KCC 창호 대리점 특성상 자재 사양이나 세부 요구사항을 수정하여 Gemini AI에게 설명글을 매끄럽게 재추천받을 수 있는 인터랙션이 부재했습니다. 또한, AI 모델이 설명글을 생성하는 도중(약 3~5초 소요) 관리자가 브라우저 뒤로가기(`popstate`)를 누르거나 새로고침(`beforeunload`)을 시도하여 전체 로딩창이 켜진 채 비정상적으로 페이지를 이탈하는 불안정한 UX가 발생했습니다.

**결정:**

1. **5단계 원스톱 AI 재생성 및 피드백 반영**: `AiFeedbackPanel` 컴포넌트를 완전히 분리하고 `useRef`를 사용하여 실시간 피드백 입력 시 발생하는 불필요한 부모 컴포넌트의 리렌더링을 0회로 격리했습니다. 관리자가 피드백(예: "더 간단하게 써줘", "단열 보강 강조해줘")을 입력하고 재생성 버튼을 누르면 자재 스펙과 피드백을 실시간 결합하여 Gemini AI로부터 설명글을 동적으로 새로 추천받도록 설계했습니다.
2. **비정상 페이지 이탈 방지 가드**: AI 생성 로딩 오버레이가 활성화된 동안 브라우저 `popstate` 이벤트와 `beforeunload` 이벤트를 강제로 리스닝 및 차단(`preventDefault`)하여 로딩 도중 뒤로가기/새로고침을 통해 UI 상태가 꼬이는 문제를 완벽하게 방지했습니다.

**이유:**

- **입력 피드백 성능 최적화**: 텍스트 입력의 부모 렌더 스코프 영향을 원천 차단하여 마법사 전체가 버벅이는 현상을 방지했습니다.
- **브라우저 히스토리 보호**: 사용자가 무심코 뒤로가기를 눌러도 AI 작업이 중간에 유실되거나 로딩 모달이 브라우저 돔에 그대로 방치되는 교착 상태(Stale State)를 UX적으로 원천 차단했습니다.

---

## ADR-017: 클라이언트 이미지 업로드/압축 최적화 임계값(150KB) 및 바이패스 스킵 로직

**배경:**
기존에는 시공사례 이미지 업로드 시 브라우저단에서 일괄적으로 WebP 변환 및 리사이징 압축 처리를 진행했습니다. 그러나 150KB 이하의 이미 작은 용량의 이미지에 대해서도 매번 압축 계산 연산이 수행되어, 모바일 브라우저 환경에서 메모리 및 프로세싱 낭비가 발생하는 단점이 존재했습니다.

**결정:**

- 업로드 대상 이미지의 파일 크기가 **150KB** 이하인 경우에는 리사이징 및 압축 단계를 건너뛰고(Bypass), 오직 WebP 컨테이너 변환만 수행하여 Storage로 즉시 전송하도록 최적화했습니다.
